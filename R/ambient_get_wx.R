...
#' @param the_date Date or character string in yyyy-mm-dd format. Ending date (UTC) for requested observation(s).
...
#' }
ambient_get_wx <- function(n=1, the_date = Sys.Date(), device_mac_address = Sys.getenv("AMBIENT_MAC_ADDRESS"), app_key = Sys.getenv("AMBIENT_APP_KEY"), api_key = Sys.getenv("AMBIENT_API_KEY")){
  if(nchar(device_mac_address) < 2) {
    ...
    parsed$datelocaltime <- lubridate::as_datetime(parsed$dateutc / 1000, tz = Sys.timezone())
    parsed$dateutc <- lubridate::as_datetime(parsed$dateutc / 1000, tz = "GMT")
    lastcol <- ncol(parsed)
    parsed <- parsed[,c(1,lastcol, 2:lastcol - 1)]
    return(parsed)
  }
